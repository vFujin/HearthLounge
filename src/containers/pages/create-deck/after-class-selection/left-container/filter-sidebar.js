...
const FilterSidebar = ({faction, filtersView, mechanics, name, query, race, type, cardName, cardRace, cardMechanics, cardFaction, cardType, cardHealth, cardAttack, cardDurability, setStandard, setWild, setTopbar, setCost, rarity, updateFilter}) => {
  ...
  return (
      <div className={`sidebar__body ${filtersView === true ? 'active' : 'display-none'}`}>
        <InputFilter attribute={name}      value={cardName}      filter="name"      multiple={false} handleInputChange={handleInputChange}/>
        <InputFilter attribute={race}      value={cardRace}      filter="race"      multiple={true} handleInputChange={handleInputChange}/>
        <InputFilter attribute={mechanics} value={cardMechanics} filter="mechanics" multiple={true} handleInputChange={handleInputChange}/>
        <InputFilter attribute={faction}   value={cardFaction}   filter="faction"   multiple={true} handleInputChange={handleInputChange}/>
        <InputFilter attribute={type}      value={cardType}      filter="type"      multiple={true} handleInputChange={handleInputChange}/>
