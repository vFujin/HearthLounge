...
const FormSelect = ({
  ...
  const mapAdventures = () => {
    return _.map(extension_details.adventures, adventure => (
      <Option value={adventure.name} title={adventure.name} key={adventure.url}>
  ...
  const mapBosses = () => {
    const wing = extension_details.adventures.find(
      adventure => adventure.name === deckAdventure
    ).wings.details;
    const bosses = wing =>
      wing.bosses.map(boss => (
        <Option value={boss.name} key={boss.name}>
          {boss.name}
        </Option>
      ));
  ...
  const mapDefault = () => {
    return icon_filters[section].map(el => (
      <Option value={isArchetype(el)} key={el.name}>
