...
import { Avatar } from 'hearthlounge-design-system';
...
import 'hearthlounge-design-system/lib/styles.css';

const DeckList = ({ decks, windowWidth, handleDeckSnippetClick }) => {
  const mapDecks = () =>
    decks.all.map(deckObj => {
      if (windowWidth <= 815) {
        return (
          <li className="deckSnippet__wrapper" key={deckObj.deckId}>
            <DeckSnippet d={deckObj} handleDeckClick={handleDeckSnippetClick} />
          </li>
        );
      } else {
        return (
          <DeckSnippetExtended
            key={deckObj.deckId}
            handleDeckSnippetClick={handleDeckSnippetClick}
            deckObj={deckObj}
          />
        );
      }
  ...
  return (
    <div
      className={`decks__decklist ${
        windowWidth <= 815 ? 'decks__decklist--mobile' : undefined
      }`}
    >
