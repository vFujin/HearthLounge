...
const Routes = ({ route, mobileMenuActive, windowWidth }) => {
  const validatePlayerClass = ({ playerClass }) => {
    if (localStorage.hearthloungeGameInfo) {
      ...
      const standardPlayerClasses = JSON.parse(
        localStorage.hearthloungeGameInfo
      ).classes;
      const filterPlayerClasses = standardPlayerClasses
        .filter(pc => !notStandardPlayerClasses.includes(pc))
        .map(pc => pc.toLowerCase());
  ...
  return (
    <div
      className={
        mobileMenuActive ? 'switch__wrapper--mobileMenuActive' : undefined
      }
      style={{ overflow: 'auto' }}
    >
      <Switch>
        ...
        <Route
          path="/extensions/:extension/:details"
          render={routeObj =>
        ...
        <Route
          path="/create-deck/:playerClass"
          render={routeObj => validatePlayerClass(routeObj.match.params)}
        />
