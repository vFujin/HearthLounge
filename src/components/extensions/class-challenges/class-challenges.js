...
export default ({
  ...
  const classChallenge = extension.class_challenges.find(
    challenge => challenge.playerClass === detailsChild
  );

  const listClassChallengeCards = () => {
    return extension.class_challenges.map(challenge => (
      <li
        key={challenge.playerClass}
        className={`${
          challenge.playerClass
        } active-on-hover ${challenge.playerClass === detailsChild &&
          'active'}`}
      >
        <Link
          to={`/extensions/${extension.url}/${classChallengeType}/${challenge.playerClass}`}
        >
